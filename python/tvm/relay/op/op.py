...
def _create_fstrategy_from_schedule(op_name, schedule, tir_schedule=None):
    ...
    fstrategy.set_default(
        _wrap_default_fstrategy(compute, schedule.fdefault, _tir_schedule, name="%s.generic" % name_pfx)
    )
...
def register_injective_schedule(op_name, level=10):
    ...
    level : int
        The priority level
    """
    return register_schedule(op_name, _schedule_injective, _tir_schedule_injective, level)
